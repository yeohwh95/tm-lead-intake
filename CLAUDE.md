...
⚠️ **KNOWN STALE-CONTENT RISK (flagged 2026-07-20, NOT yet fixed):** the TESTRIDE branch (`FR_EVENT_INFO` env, "test ride event kami 17 & 18 July... Zontes dan KTM") is now telling customers about an event that has **already passed** — a customer asked about test rides today (07-20) and got invited to the 17-18 July event. Benjamin is waiting on the TM team for the correct next answer before updating — **do not guess new wording; wait for their input** before touching `FR_EVENT_INFO` or the testride reply.
